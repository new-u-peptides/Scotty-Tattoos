...
(function () {
  ...
  function centerMotif(s, R, rand) {
    ...
    if (R < 0.05) { polyS(s, 0, 0, Math.max(0.03, R), 4, Math.PI / 4, STR); return; }
    if (t < 0.22) { triangleS(s, Rm, true, STR); triangleS(s, Rm, false, STR); }                  // hexagram
    else if (t < 0.40) { polyS(s, 0, 0, Rm, 4, 0, STR); polyS(s, 0, 0, Rm, 4, Math.PI / 4, STR); } // 8-point star
    else if (t < 0.58) { triangleS(s, Rm, true, STR); triangleS(s, Rm * 0.72, false, STR); triangleS(s, Rm * 0.46, true, STR); } // nested tris (yantra)
    else if (t < 0.76) { starPolyS(s, 0, 0, 5, Rm, Rm * 0.42, STR); }                              // pentagram
    else if (t < 0.90) { ringDiamonds(s, 5 + (rand() * 3 | 0), R * 0.18, Rm, STR); }               // diamond burst
    else { polyS(s, 0, 0, Rm, 6, rand() * TAU, STR); polyS(s, 0, 0, Rm * 0.6, 6, rand() * TAU, STR); } // hexagons
    // angular core — small diamonds, not circles
    polyS(s, 0, 0, R * 0.36, 4, Math.PI / 4, 0.82);
    polyS(s, 0, 0, R * 0.14, 4, Math.PI / 4, 0.95);
  }

  // One random geometric mandala → a strokes array. Deliberately ANGULAR:
  // polygon frames and separators instead of circles, and every band is a
  // sharp-edged element (diamonds, triangles, squares, chevrons, star
  // polys, nested polygons). It reads as a faceted mandala that morphs.
  function randomMandala(rand) {
    var s = [], STR = 0.85;
    function pick(arr) { return arr[(rand() * arr.length) | 0]; }
    // angular outer frame — two concentric polygons, no circles
    var frameN = pick([6, 8, 10, 12]);
    polyS(s, 0, 0, 0.985, frameN, rand() * TAU, 0.7);
    polyS(s, 0, 0, 0.95, frameN, rand() * TAU, 0.78);
    var r = pick([0.90, 0.92, 0.94]);
    var bands = 4 + (rand() * 3 | 0);                 // 4–6 bands
    for (var b = 0; b < bands; b++) {
      var rIn = r - (0.09 + rand() * 0.07);
      if (rIn < 0.13) rIn = 0.13;
      var n = pick([6, 8, 10, 12, 12, 16, 18]);
      var mid = (rIn + r) / 2, t = rand();
      // all-angular band elements — no circle/petal rings
      if (t < 0.22) ringDiamonds(s, n, rIn, r, STR);
      else if (t < 0.36) diamondLattice(s, n, rIn, r, STR);          // dense diamonds
      else if (t < 0.50) ringTriangles(s, n, rIn, r, rand() < 0.6, STR);
      else if (t < 0.62) ringSpikes(s, n, rIn, r, STR);              // sharp points
      else if (t < 0.74) { var rot = rand() * TAU; polyS(s, 0, 0, r, n, rot, STR); polyS(s, 0, 0, rIn, n, rot + Math.PI / n, STR); } // nested polygons
      else if (t < 0.85) ringSquares(s, n, mid, (r - rIn) * 0.5, rand() * Math.PI, STR);
      else if (t < 0.93) ringChevrons(s, n, rIn, r, STR);
      else starPolyS(s, 0, 0, n, r, rIn, STR);                       // big sharp star ring
      // angular separator — a polygon outline at the band base (not a circle)
      polyS(s, 0, 0, rIn, pick([6, 8, 12]), rand() * TAU, 0.7);
      r = rIn - rand() * 0.015;
      if (r < 0.15) break;
    }
